...
<br/>
   <p class="date col-sm-2 col-md-2" >
      ...
      <p class="col-sm-2 col-md-2" id="categorylink">
       <a href="{{url('CAMPAIGNS')}}">{{ trans('home.CAMPAIGNS')}}</a>
      </p>
...
@foreach($relatedposts as $relatedpost)
  <div class="col-sm-4" >
      <a href="{{url(date_format($relatedpost->created_at,'Y/m/d').'/'.$relatedpost->slug)}}" >
        @php $relatedpost->image = str_replace('\\','/',$relatedpost->image) @endphp
        @if (strpos($relatedpost->image, 'http') !== false)
         <div id="attacksbox" style="background: url({{$relatedpost->image}})">
        @else
      <div id="attacksbox" style="background: url({{url('storage/'.$relatedpost->image)}})">
        @endif
     <p class="caption">
       
       
      {{ \Illuminate\Support\Str::limit($relatedpost->title, 60, $end='...') }}
