...
<br/>
   <p class="date col-sm-2 col-md-2" >
      ...
      <p class="col-sm-2 col-md-2" id="categorylink">
            <a href="{{url('news')}}">{{ trans('home.NEWS')}}</a>
      </p>
...
@foreach($relatedposts as $relatedpost)
 <a href="{{url(date_format($relatedpost->created_at,'Y/m/d').'/'.$relatedpost->slug)}}" class="col-sm-4">
   <div>
    <div id="newsbox">
     
    @php $relatedpost->image = str_replace('\\','/',$relatedpost->image) @endphp
    @if (strpos($relatedpost->image, 'http') !== false)
     <div id="homeimgbox" style="background: url({{$relatedpost->image}})">
     </div>
     @else
    <div id="homeimgbox" style="background: url({{url('storage/'.$relatedpost->image)}})">
     </div>
     @endif
     ...
     <p class="caption">
      ...
      {{ \Illuminate\Support\Str::limit($relatedpost->title, 45, $end='...') }}
